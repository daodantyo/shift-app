...
export default function ShiftRequestForm() {
  ...
  // 送信控えの文面を作る(お店・本人どちらにも同じ内容を送る)
  const buildReceiptText = () => {
    const WD = ["日", "月", "火", "水", "木", "金", "土"];
    const name = (selectedCast && selectedCast.name) || "";
    const lines = [];
    lines.push("🌸 希望シフトを受け付けました");
    lines.push("");
    lines.push(name + " さん");
    lines.push("");
    dates.forEach((d) => {
      const e = entries[d.toDateString()] || { status: "off" };
      const label = (d.getMonth() + 1) + "/" + d.getDate() + "(" + WD[d.getDay()] + ")";
      if (e.status === "work" && e.in && e.out) {
        lines.push(label + " " + e.in + "〜" + e.out);
      } else {
        lines.push(label + " 休み");
      }
    });
    lines.push("");
    const now = new Date();
    lines.push("送信日時：" + (now.getMonth() + 1) + "/" + now.getDate() + " " +
      String(now.getHours()).padStart(2, "0") + ":" + String(now.getMinutes()).padStart(2, "0"));
    return lines.join("\n");
  };

  // お店(管理者)と本人に控えLINEを送る。失敗しても提出自体は成功扱いにする(控えはオマケ)
  const sendReceipts = async () => {
    ...
    // 本人へ(LINE IDが取れているときだけ)
    if (profile?.userId) {
      try {
        await fetch("/api/send-line", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ messages: [{ to: profile.userId, text }] }),
        });
      } catch (e) {}
    }
  };
  ...
  const submit = () => {
    ...
    push(requestsRef, {
      ...
      .then(async () => {
        // 保存が成功したら、お店と本人に控えを送る(控えの成否は問わない)
        await sendReceipts();
        setSubmitting(false);
        setSubmitted(true);
      })
